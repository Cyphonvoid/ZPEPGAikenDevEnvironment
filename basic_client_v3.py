...
class BareClient:
    def __init__(
        ...
        # Stage 2 of _confirm(): waiting for the address-level UTXO listing
        # to catch up after transaction-level confirmation. Measured
        # empirically (measure_lag_auto.py, 15 cycles) at ~2s / 1 poll in
        # 14/15 cases - this window is generous headroom above that.
        self.ADDRESS_CATCHUP_TIMEOUT_S = 20.0
        self.ADDRESS_CATCHUP_POLL_INTERVAL_S = 2.0
        # Stage 3 of _confirm(): waiting for the funding address's own
        # UTXO listing to catch up (its change output from this same tx).
        self.FUNDING_CATCHUP_TIMEOUT_S = 20.0
        self.FUNDING_CATCHUP_POLL_INTERVAL_S = 2.0
    ...
    def _confirm(self, tx_hash: str):
        """
        Three-stage confirmation, based on empirically measured Blockfrost
        behavior: the per-transaction endpoint, the script-address UTXO
        listing, and the funding-address UTXO listing are three SEPARATE
        indexes that do not update atomically together.

        Stage 1: poll transaction_utxos(hash=tx_hash) until it succeeds -
                 proves the transaction is confirmed on-chain.

        Stage 2: poll _get_master_utxo() until it returns the UTXO created
                 by tx_hash - proves the script-address view has caught up.

        Stage 3: poll the funding address until one of its UTXOs was
                 created by tx_hash (the wallet's own change output) -
                 proves the funding-address view has caught up too, so the
                 next write's collateral/fee selection won't reference a
                 UTXO this write already spent.

        Only returns success once all three stages pass.
        """
        ...
        if not tx_confirmed:
            return False, f"Transaction-level confirmation timeout after {self.CONFIRMATION_TIMEOUT_S}s. Last error: {last_err}"

        # Stage 2: script-address (master UTXO) view catch-up
        addr_deadline = time.monotonic() + self.ADDRESS_CATCHUP_TIMEOUT_S
        script_caught_up = False
        while time.monotonic() < addr_deadline:
            try:
                master_utxo, _ = self._get_master_utxo()
                if str(master_utxo.input.transaction_id) == tx_hash:
                    script_caught_up = True
                    break
            except Exception as e:
                last_err = str(e)
            time.sleep(self.ADDRESS_CATCHUP_POLL_INTERVAL_S)

        if not script_caught_up:
            return False, (
                f"Transaction {tx_hash} confirmed at the tx level, but the "
                f"script address's UTXO listing did not reflect it within "
                f"{self.ADDRESS_CATCHUP_TIMEOUT_S}s. Last error: {last_err}"
            )

        # Stage 3: funding-address view catch-up
        funding_deadline = time.monotonic() + self.FUNDING_CATCHUP_TIMEOUT_S
        while time.monotonic() < funding_deadline:
            try:
                funding_utxos = self.context.utxos(self.funding_address)
                if any(str(u.input.transaction_id) == tx_hash for u in funding_utxos):
                    return True, None
            except Exception as e:
                last_err = str(e)
            time.sleep(self.FUNDING_CATCHUP_POLL_INTERVAL_S)

        return False, (
            f"Transaction {tx_hash} confirmed and the script address caught up, "
            f"but the funding address's UTXO listing did not reflect this "
            f"transaction's change output within {self.FUNDING_CATCHUP_TIMEOUT_S}s. "
