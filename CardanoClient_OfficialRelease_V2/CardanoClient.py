...
from CardanoUtils import (
    ...
    VerificationKeyCredential, ScriptCredential, TokenDatum,
    MintDocument, MintToken, Pause, Resume, RotateKey,
    LinkForward, LinkBackward, SweepFunds,
    AuthorityKeyTag, OperatorKeyTag, OwnerKeyTag,
    PlutusAddress, NoneStakeCredential,
)
...
class CardanoClient:
    ...
    # ══════════════════════════════════════════════════════════════════════
    def _confirm_sweep(self, tx_hash: str, swept_utxos: list) -> tuple[bool, Optional[str], Optional[str]]:
        deadline = time.monotonic() + self.CONFIRMATION_TIMEOUT_S
        last_err = None
        swept_refs = {f"{str(u.input.transaction_id)}#{u.input.index}" for u in swept_utxos}
        ...
        # Stage 2: wait for swept UTxOs to disappear from script address
        addr_deadline = time.monotonic() + self.ADDRESS_CATCHUP_TIMEOUT_S
        while time.monotonic() < addr_deadline:
            try:
                current = self._backend.utxos(self._script_address)
                current_refs = {f"{str(u.input.transaction_id)}#{u.input.index}" for u in current}
                if not swept_refs.intersection(current_refs):
                    break
            ...
            return False, (
                f"Swept UTxOs did not disappear from script address within "
    ...
    # Public: withdraw
    # ══════════════════════════════════════════════════════════════════════
    def withdraw_extra_funds(
        self,
        to_address: str,
        amount_lovelace: int,
    ) -> dict:
        """
        Sweep non-citizen plain ADA UTxOs from the script address to any
        specified address. Only plain ADA UTxOs with no datum are swept —
        the contract physically rejects any input carrying tokens under
        this policy (master UTXO or document token UTxOs).

        Any of the three keys (authority, operator, owner) can authorize.
        The master UTXO is passed as a reference input — never spent.

        Args:
            to_address:      Bech32 destination address string.
            amount_lovelace: Amount in lovelace to sweep. Must be positive.
        """
        try:
            if amount_lovelace <= 0:
                return _failure(
                    "withdraw_extra_funds",
            ...
            # ── Find plain ADA non-citizen UTxOs at script address ────────
            all_script_utxos = self._backend.utxos(self._script_address)
            plain_utxos = [
                u for u in all_script_utxos
                if not u.output.amount.multi_asset
                and u.output.datum is None
                and str(u.input.transaction_id) != self._ref_script_tx_hash
            ]
            if not plain_utxos:
                return _failure(
                    "sweep_funds",
                    "No plain ADA non-citizen UTxOs found at script address.",
                    ERROR_INVALID_INPUT,
                )

            # ── Parse destination address ─────────────────────────────────
            dest_address = Address.from_primitive(to_address)

            from pycardano import VerificationKeyHash as PyVKH
            payment_part = dest_address.payment_part
            if isinstance(payment_part, PyVKH):
                pay_cred = VerificationKeyCredential(bytes(payment_part))
            else:
                pay_cred = ScriptCredential(bytes(payment_part))

            dest_plutus_address = PlutusAddress(
                payment_credential=pay_cred,
                stake_credential=NoneStakeCredential(),
            )
            ...
            redeemer = SweepFunds(
                ...
                to_address=dest_plutus_address,
                signature=signature,
            )

            dest_output = TransactionOutput(
                address=dest_address,
                amount=amount_lovelace,
            )

            # ── Build — spend plain UTxOs, reference master UTXO ─────────
            builder = self._new_builder()
            builder.add_input_address(self._funding_address)

            # Add plain non-citizen UTxOs as script inputs
            for u in plain_utxos:
                builder.add_script_input(utxo=u, redeemer=Redeemer(redeemer))

            # Master UTXO as reference input only
            builder.reference_inputs.add(master_utxo.input)
            builder.reference_inputs.add(ref_utxo.input)

            self._attach_collateral(builder)
            builder.add_output(dest_output)
            ...
            try:
                ...
                return _failure(
                    "withdraw_extra_funds", str(e), _classify_error(e),
                    nonce_used=nonce, submitted_at=submitted_at,
                    duration_s=round(time.time() - submitted_at, 2),
                )

            # Stage 1 + Stage 3 only — master UTXO not updated
            confirmed, err, err_type = self._confirm_sweep(tx_hash, plain_utxos)
            confirmed_at = time.time()

            if not confirmed:
                return _failure(
                    "withdraw_extra_funds", err, err_type,
                    nonce_used=nonce, tx_hash=tx_hash,
                    submitted_at=submitted_at,
                    duration_s=round(confirmed_at - submitted_at, 2),
                )

            return _success(
                operation="withdraw_extra_funds",
                tx_hash=tx_hash,
                fee_lovelace=fee_lovelace,
                nonce_used=nonce,
                nonce_after=nonce,  # master UTXO not spent, nonce unchanged
                submitted_at=submitted_at,
                confirmed_at=confirmed_at,
                extra={
                    ...
                    "plain_utxos_swept":  len(plain_utxos),
                },
            )

        except Exception as e:
            return _failure("withdraw_extra_funds", str(e), _classify_error(e))
