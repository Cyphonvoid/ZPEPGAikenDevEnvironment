...
SINGLE-SCRIPT ARCHITECTURE: registry_contract.ak's archive_registry
validator is now parameterized by BOTH genesis_ref AND beacon_asset_name
(formerly genesis_ref alone parameterized a separate beacon_contract.ak).
BEACON_ASSET_NAME below is still a deploy-time constant exactly as before
- it's just threaded into one combined apply_parameters() call now instead
of being baked into a standalone beacon compilation stage.
"""
    ...
    def build_parser(cls) -> argparse.ArgumentParser:
        parser = argparse.ArgumentParser(
            ...
            description=(
                ...
                "Handles genesis bootstrap: parameterize archive_registry "
                "with the genesis UTXO and beacon asset name, mint the "
                "beacon token, create the first master UTXO."
            ),
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            help="Output path for the fully-parameterized blueprint.",
        )
        parser.add_argument(
...
def run_deployment(
    ...
    print("\n--- Parameterizing archive_registry (genesis_ref + beacon_asset_name) ---")
    ...
    print(f"  Beacon asset name:     {BEACON_ASSET_NAME!r}")
    try:
        ...
        raise CLI.Error(str(e)) from e
    print(f"  Policy ID (shared by registry + beacon): {applied.policy_id_hex}")
    ...
    try:
        tx_result = GenesisTransaction.run(
            ...
            compiled_code_hex=applied.compiled_code_hex,
    ...
    try:
        written = DeploymentRecord.write(record, args.deployment_json, allow_overwrite=args.overwrite or True)
    except DeploymentRecord.Error as e:
        raise CLI.Error(str(e)) from e

    print(f"  Written: {written}")
    print("\n=== Deployment complete ===")
    print(f"  Policy ID (registry + beacon): {tx_result.policy_id_hex}")
    print(f"  Registry address:              {tx_result.registry_script_address}")
    print(f"  Master UTXO:                   {tx_result.master_utxo_ref}")
