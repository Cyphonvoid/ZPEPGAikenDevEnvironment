...
class BareClient:
    ...
    def _confirm(self, tx_hash: str):
        """
        Two-stage confirmation, based on empirically measured Blockfrost
        behavior (see measure_lag_auto.py 15-cycle test): the per-transaction
        endpoint and the per-address UTXO-listing endpoint are NOT updated
        atomically together. transaction_utxos(hash) confirming a tx does
        NOT guarantee the very next context.utxos(script_address) call will
        already reflect that tx's new master UTXO - it lagged by exactly
        one poll cycle (~2s) in 14/15 measured cycles, always resolving
        near-instantly, never open-ended.

        Stage 1: poll transaction_utxos(hash=tx_hash) until it succeeds -
                 proves the transaction is confirmed on-chain. Unchanged
                 from the original single-stage version; this endpoint
                 itself has been reliable across all measured cycles.

        Stage 2: poll _get_master_utxo() until the UTXO it returns has
                 input.transaction_id == tx_hash - proves the address-level
                 view has caught up to the write we just made, so the next
                 write can safely build against what _get_master_utxo()
                 returns without risk of referencing stale state.

        Only returns success once BOTH stages pass.
        """
        deadline = time.monotonic() + self.CONFIRMATION_TIMEOUT_S
        last_err = None

        # Stage 1: transaction-level confirmation (unchanged)
        tx_confirmed = False
        ...
        # Stage 2: address-level view catch-up
